...
class DateHelper @Inject() (clock: Clock) {
  def ukClock: Clock                     = clock.withZone(ukZoneId)
  def nowUkLocalDate: LocalDate          = LocalDate.now(ukClock)
  def futureDateHelper: FutureDateHelper = FutureDateHelper(nowUkLocalDate)
}

object DateHelper {
  val ukZoneId: ZoneId                  = ZoneId.of("Europe/London")
  val beginningOf20thCentury: LocalDate = LocalDate.of(1900, 1, 1)
}
