...
object CertificateAdditionalInformationSummary {
  ...
  def row(answers: UserAnswers)(using messages: Messages): Option[SummaryListRow] =
    answers.get(CertificateAdditionalInformationPage).map {
      answer =>
        SummaryListRowViewModel(
          key     = messages("certificateAdditionalInformation.checkYourAnswersLabel").toKey,
          value   = ValueViewModel(answer.toText),
          actions = Seq(
            ActionItemViewModel(messages("site.change").toText, routes.CertificateAdditionalInformationController.onPageLoad(CheckMode).url)
