...
trait TextareaFluency {
  ...
  object TextareaViewModel extends ErrorMessageAwareness {
    ...
    def apply(
        field: Field,
        label: Label
    )(using messages: Messages): Textarea =
      Textarea(
        id = field.id,
        name = field.name,
        value = field.value,
  ...
  extension (textarea: Textarea) {
    ...
    def withFormGroup(formGroup: FormGroup): Textarea =
      textarea.copy(formGroup = formGroup)
    ...
    def withSpellcheck(on: Boolean): Textarea =
