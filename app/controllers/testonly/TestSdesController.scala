...
// sm2 --start SDES_PROXY SDES_STUBS
class TestSdesController @Inject() (
  ...
  def fileReady: Action[AnyContent] = Action.async { implicit request =>
    def sdesRequestBody(md5: String, contentLength: Long) = Json.obj(
    ...
    for {
      ...
      // We can get the md5 & contentLength from the PUT to object store instead of having to make this call.
      (md5, contentLength) <- objectStoreClient
        .presignedDownloadUrl(path = objectStorePath, owner = TestObjectStoreController.service)
        .map(r => (r.contentMd5, r.contentLength))
      sdesResponse <- httpClientV2
        ...
        .withBody(
          sdesRequestBody(md5 = md5.value, contentLength = contentLength)
        )
      ...
      logger.error(s"[SDES-fileReady][contentLength]=$contentLength")
      logger.error(
        s"[SDES-fileReady][json]=${Json.stringify(sdesRequestBody(md5 = md5.value, contentLength = contentLength))}"
      )
