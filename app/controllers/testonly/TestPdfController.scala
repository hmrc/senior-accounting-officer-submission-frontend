...
class OpenHtmlToPdfService {
  def testPdf(): PdfRendererBuilder = {
    ...
    builder.useFont(
      File(this.getClass.getResource("/fonts/SFNS.ttf").getFile),
      "SFNS Display"
    )
...
object TestPdfController {
  ...
  // <html lang="en"> changes the pdf language meta data, otherwise it's EN-US by default
  def testHtml: String =
    """
       |
       |<!DOCTYPE html>
      ...
      + (0 until 5000)
        .map(index => s"""
                     |  <tr>
                     |    <td>Alfreds Futterkiste ${index * 2 + 1}</td>
                     |    <td>Maria Anders</td>
                     |    <td>Germany</td>
                     |  </tr>
                     |  <tr>
