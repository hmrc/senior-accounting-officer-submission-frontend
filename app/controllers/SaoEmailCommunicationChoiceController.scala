...
class SaoEmailCommunicationChoiceController @Inject() (
    ...
    formProvider: SaoEmailCommunicationChoiceFormProvider,
    val controllerComponents: MessagesControllerComponents,
    view: SaoEmailCommunicationChoiceView
    ...
    implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(SaoEmailCommunicationChoicePage, value))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(SaoEmailCommunicationChoicePage, mode, updatedAnswers))
        )
  }
}
