...
class $className$SummarySpec extends SpecBase with GuiceOneAppPerSuite {
  ...
  "$className$Summary.row" - {
    ...
    "when there is a user answer for $className$Page" - {
      def testUserAnswers(answers: Set[$className$]) =
        emptyUserAnswers.set($className$Page, answers).get

      def SUT(answers: Set[$className$] = Set.empty) = $className$Summary.row(testUserAnswers(answers)).get
      ...
      "expected value" - {
        "must show '$option1msg$' when user answers is $option1key;format="Camel"$" in {
          SUT(answers = Set($className$.$option1key;format="Camel"$)).value.content mustBe HtmlContent("$option1msg$")
        }

        "must show '$option2msg$' when user answers is $option2key;format="Camel"$" in {
          SUT(answers = Set($className$.$option2key;format="Camel"$)).value.content mustBe HtmlContent("$option2msg$")
        }
      }
