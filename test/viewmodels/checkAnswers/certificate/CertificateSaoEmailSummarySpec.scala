...
class CertificateSaoEmailSummarySpec extends SpecBase with GuiceOneAppPerSuite {
  ...
  "CertificateSaoEmailSummary.row" - {

    "when there is no answer for CertificateSaoEmailPage" - {
      "must return None" in {
        def SUT = CertificateSaoEmailSummary.row(emptyUserAnswers)

        SUT mustBe None
      }
    }

    "when there is a user answer for CertificateSaoEmailPage" - {
      def testUserAnswers(answer: String) =
        emptyUserAnswers.set(CertificateSaoEmailPage, answer).get
      ...
      "expected action" - {
        ...
        "must have expected url" in {
          action.href mustBe certificateRoutes.CertificateSaoEmailController
