...
class NotificationUploadFormControllerSpec extends SpecBase with MockitoSugar {
  ...
  "NotificationUploadFormController must" - {
    ...
    "return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, notificationRoutes.NotificationUploadFormController.onPageLoad().url)
      ...
      running(application) {
        ...
        verify(mockSessionRepository, times(1)).set(
          argThat { (answers: models.UserAnswers) =>
            ...
            answers
              .get(notification.NotificationUploadStatePage)
              .contains(
      ...
      val application = applicationBuilder(userAnswers =
        Some(
          completedSaoDetailsAnswers
            .set(NotificationUploadStatePage, FileUploadState("foo", uploadStatus))
            .get
        )
      )
        .overrides(
