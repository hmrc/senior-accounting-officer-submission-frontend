...
class UnqualifiedCompaniesControllerSpec extends SpecBase {
  ...
  "UnqualifiedCompanies Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, certificateRoutes.UnqualifiedCompaniesController.onPageLoad().url)
    ...
    "must redirect to the next page for a POST" in {
      ...
      running(application) {
        val request = FakeRequest(POST, certificateRoutes.UnqualifiedCompaniesController.onSubmit().url)
    ...
    "must redirect to Journey Recovery for a POST if no existing data is found" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, certificateRoutes.UnqualifiedCompaniesController.onSubmit().url)
