...
import views.html.certificate.CertificateAdditionalInformationView
...
class CertificateAdditionalInformationControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val certificateAdditionalInformationRoute: String =
    certificateRoutes.CertificateAdditionalInformationController.onPageLoad(NormalMode).url
  ...
  "CertificateAdditionalInformation Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[CertificateAdditionalInformationView]
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, certificateAdditionalInformationRoute)
    ...
    "must redirect to task list on upload submission template stage when user answers is empty for a POST" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, certificateRoutes.CertificateAdditionalInformationController.onSubmit(NormalMode).url)
